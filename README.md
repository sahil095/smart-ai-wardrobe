...
- **Standardized "retail" images:** uploads are background-removed and padded
  onto a constant background (a uniform product-catalog look) via Cloudinary
  on-the-fly transformations. The original is always kept; each item stores a
  `display_image_url`, with an automatic fallback to the original if the
  transform can't render.
  - Requires **AI Background Removal enabled on your Cloudinary account**. If it
    isn't available (or you prefer originals), set `STANDARDIZE_IMAGES=false`.
  - Backfill older items: `python -m scripts.backfill_display`.
- **Installable PWA:** add Wardrobe AI to your phone's home screen (manifest +
  service worker + app icon), with an offline app shell.
- **AI-generated, category-dependent attributes & filters:** picking a category
  reveals extra, relevant fields (e.g. Shoes -> Sole / Closure / Height). The
  attribute set is generated by Groq, cached per category in the database, and
  falls back to built-in defaults. These attributes become facet filters on the
  Wardrobe page. Only relevant base fields show per category (e.g. Sleeve/Neck
  for tops).
